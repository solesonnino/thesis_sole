    ...
    static public void Main(ref StringWriter output)
    {
        ...
        try
        {
            ...
            for (int i=0; i<N; i++)
            {
                ...
                //ricevi pick
                var pick_rec= ReceiveNumpyArray(stream);
                var pick= pick_rec[0,0];                
                //fai calcoli 
                //move the base of the robot in the defined position 
		                                ...
		                                GetObjectsByName("Cube_1"+pick.ToString())[0];
                                        ...
                                        selectedObject = TxApplication.ActiveDocument.GetObjectsByName("Cube_1"+pick.ToString());
                ...
                //manda fitness
                string fitness_string = (fitness_int).ToString();
                byte[] byte_fitness = Encoding.ASCII.GetBytes(fitness_string);
                stream.Write(byte_fitness, 0, byte_fitness.Length);
